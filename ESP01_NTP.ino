/**
 * Network Time Protocol - ESP01
 * 
 * This module fetch time packet from NTP server and convert it in UNIX time.
 * Unix timestamp is used to get current date and time. I have added Indian GMT 
 * in final long value. You can add any server of your choice in NTP server list. 
 * It's safe side to keep more than one server. There is no code written in loop  
 * as we are giving output one time only. 
 * 
 * This is working example and tested with ESP-01 module.
 * FTDI adapter is used to flash ESP8266 chip. Use 3.3v jumper on FTDI adapter.
 * You can use Arduino Uno instead of FTDI, but use voltage divider resistors to 
 * convert Arduino's 5v to 3.3v.
 * 
 * Code and related information on website is given as it is, you can use it on 
 * your own choice. 
 * 
 * @author Amit P amitofiles@gmail.com
 * @since 20181215
 * 
 */
